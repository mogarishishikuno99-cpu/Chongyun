...
//   Photo   : GoatBot V2 usersData.getAvatarUrl() (Sans Token FB)
// ═══════════════════════════════════════════════════════════════════════════════
...
// Fallback sécurisé si fonts.js n'est pas chargé
const f = fonts || {
  bold: t => t,
  italic: t => t,
  mono: t => t,
...
if (canvasAvailable && registerFont) {
  ...
  [["BeVietnamPro-Bold.ttf","BF","bold"],["BeVietnamPro-Regular.ttf","BF","normal"],
   ...
   ["NotoSans-Regular.ttf","BF","normal"]].forEach(([fName,fam,w]) => {
    try { const fp = path.join(fd,fName); if(fs.existsSync(fp)) registerFont(fp,{family:fam,weight:w}); } catch(_){}
  });
}
...
async function AVATAR(ctx,avatarUrl,cx,cy,R,t){
  for(let i=0;i<3;i++){
  ...
  if(avatarUrl){
    try{
      loadedImg = await loadImage(avatarUrl);
    }catch(_){}
  }
  if(loadedImg){
    ctx.save();ctx.beginPath();ctx.arc(cx,cy,R,0,Math.PI*2);ctx.clip();
    ctx.drawImage(loadedImg,cx-R,cy-R,R*2,R*2);ctx.restore();
  } else {
    AVFB(ctx,cx,cy,R,"?",t);
  }
...
async function buildCanvas(data,theme,avatarUrl){
  ...
  await AVATAR(ctx,avatarUrl,avCX,avCY,avR,theme);
  ...
  T(ctx,`◈  ${now}   ◆   Shade   ◈`,CW/2,FT_Y,14,theme.muted,{align:"center",weight:"600"});
...
Taxes de transfert :
  ...
  onStart: async function ({ message, event, args, usersData, api }) {
    ...
    if (command === "top") {
      ...
      if (!slice.length) return message.reply(f.christus("◆  Aucun utilisateur sur cette page."));
      ...
      return message.reply(f.christus(txt));
    }
    ...
    if (["transfer","send","pay","t","virement"].includes(command)) {
      ...
      if (targetID===senderID) return message.reply(f.christus("◆  Vous ne pouvez pas vous envoyer de l argent."));
      if (amount<ECONOMY.transfer.min) return message.reply(f.christus(`◆  Montant minimum : ${formatMoney(ECONOMY.transfer.min)}`));
      if (amount>ECONOMY.transfer.max) return message.reply(f.christus(`◆  Montant maximum : ${formatMoney(ECONOMY.transfer.max)}`));
      const [sender,receiver] = await Promise.all([usersData.get(senderID),usersData.get(targetID)]);
      if (!receiver) return message.reply(f.christus("◆  Destinataire introuvable."));
    ...
    if (command === "themes" || command === "theme") {
      ...
      if (args[1]) {
        ...
        if (THEMES[key]) {
          ...
          return message.reply(f.christus(`◈  Theme défini : ${THEMES[key].sym}  ${THEMES[key].name}`));
        }
      }
    ...
    const userDataRaw = await usersData.get(senderID);
    ...
    if (!userData) return message.reply(f.christus("◆  Utilisateur introuvable."));
    ...
    // Avatar via GoatBot V2 usersData.getAvatarUrl() (Sans token FB)
    let effectiveAv = null;
    try {
      effectiveAv = await usersData.getAvatarUrl(targetID);
    } catch (_) {}

    const outPath = path.join(__dirname, "cache", `bal_out_${Date.now()}.png`);
