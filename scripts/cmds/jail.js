...
module.exports = {  
  ...
  onStart: async function ({ event, message, usersData, getLang }) {    
    try {      
      ...
      if (!targetID) return message.reply(fonts.christus(getLang("noTarget")));      
      ...
      if (!apiBase) return message.reply(fonts.christus("API base URL not found in ApiUrl.json."));      
      ...
      await message.reply({        
        body: fonts.christus(`🚔 ${userInfo} is now behind bars!`),        
        attachment: fs.createReadStream(imgPath)      });      
      
      fs.unlinkSync(imgPath);    
    } catch (err) {      
      console.error(err);      
      message.reply(fonts.christus("Failed to generate jail image. Please try again later."));     
    }  
};
