...
module.exports = {  
  ...
  onStart: async function ({ message, event }) {    
    ...
    await message.reply({      
      body: fonts.christus(`🎮 𝗡𝗔𝗥𝗨𝗧𝗢-𝗦𝗧𝗢𝗥𝗠 𝗩𝟰.𝟮\n━━━━━━━━━━━━━━\n𝗘𝗻𝘃𝗼𝘆𝗲𝘇 "start" 𝗽𝗼𝘂𝗿 𝗰𝗼𝗺𝗺𝗲𝗻𝗰𝗲𝗿`),      
      attachment: await global.utils.getStreamFromURL("https://files.catbox.moe/ryh5mq.png")    });  
  },  
  onChat: async function ({ event, message, usersData }) {    
    ...
    if (state.step !== "waiting_start" && state.step !== "choose_p1" && state.step !== "choose_p2" &&         userID !== state.players.p1 && userID !== state.players.p2) {      
      return;    }    
    if (body === 'fin') {      
      delete gameState[threadID];      
      return message.reply(fonts.christus("🔄 Partie terminée. Envoyez 'start' pour recommencer."));    }    
    if (state.step === "waiting_start" && body === "start") {      
      state.step = "choose_p1";      
      state.players.p1 = userID;      
      return message.reply(fonts.christus("🧙 𝗝𝗼𝘂𝗲𝘂𝗿 𝟭\n𝗧𝗮𝗽𝗲𝘇 'p1' 𝗽𝗼𝘂𝗿 𝘀é𝗹𝗲𝗰𝘁𝗶𝗼𝗻𝗻𝗲𝗿 𝘃𝗼𝘁𝗿𝗲 𝗽𝗲𝗿𝘀𝗼𝗻𝗻𝗮𝗴𝗲"));    }    
    if (state.step === "choose_p1" && body === 'p1') {      
      if (userID !== state.players.p1) return;      
      state.step = "choose_p2";      
      return message.reply(fonts.christus("🧝 𝗝𝗼𝘂𝗲𝘂𝗿 𝟮\n𝗧𝗮𝗽𝗲𝘇 'p2' 𝗽𝗼𝘂𝗿 𝘃𝗼𝘂𝘀 𝗶𝗻𝘀𝗰𝗿𝗶𝗿𝗲"));    }    
    if (state.step === "choose_p2" && body === 'p2') {      
      if (userID === state.players.p1) {        
        return message.reply(fonts.christus("❌ Vous ne pouvez pas être les deux joueurs !"));      }      
      ...
      return message.reply({        
        body: fonts.christus(characterList + `\n\n${userInfo.name} 𝗝𝗼𝘂𝗲𝘂𝗿 𝟭, 𝗿é𝗽𝗼𝗻𝗱𝗲𝘇 𝗮𝘃𝗲𝗰 𝗹𝗲 𝗻𝘂𝗺é𝗿𝗼 𝗱𝘂 𝗽𝗲𝗿𝘀𝗼𝗻𝗻𝗮𝗴𝗲`),        
        mentions: [{          
          tag: `${userInfo.name}`,          
          id: state.players.p1        }]      });    }    
    if (state.step.startsWith("choose_characters")) {      
      ...
      if (isNaN(index) || index < 0 || index >= characters.length) {        
        return message.reply(fonts.christus("❌ 𝗡𝘂𝗺é𝗿𝗼 𝗶𝗻𝘃𝗮𝗹𝗶𝗱𝗲. 𝗥é𝗲𝘀𝘀𝗮𝘆𝗲𝘇"));      }      
      if (state.step === "choose_characters_p1" && userID === state.players.p1) {        
        ...
        return message.reply({          
          body: fonts.christus(`✅ 𝗝𝗼𝘂𝗲𝘂𝗿 𝟭: ${state.p1Character.name}\n\n${userInfo.name} 𝗝𝗼𝘂𝗲𝘂𝗿 𝟮, 𝗰𝗵𝗼𝗶𝘀𝗶𝘀𝘀𝗲𝘇 𝘃𝗼𝘁𝗿𝗲 𝗽𝗲𝗿𝘀𝗼𝗻𝗻𝗮𝗴𝗲`),          
          mentions: [{            
            tag: `${userInfo.name}`,            
            id: state.players.p2          }]        });      }      
      if (state.step === "choose_characters_p2" && userID === state.players.p2) {        
        ...
        return message.reply({          
          body: fonts.christus(battleStartMsg),          
          mentions: [{            
            tag: `${p1Info.name}`,            
            id: state.players.p1          }]        });      }      
      return;    }    
    if (state.step === "battle") {      
      ...
      if (body === 'c' && state.lastAction === 'c' && state.lastPlayer === userID) {        
        return message.reply(fonts.christus("❌ Vous ne pouvez pas charger votre chakra deux fois de suite !"));      }      
      ...
      switch (body) {        
        ...
        case 'd':          
          ...
          state.lastPlayer = userID;          
          state.turn = state.turn === "p1" ? "p2" : "p1";          
          return message.reply(fonts.christus(`🛡️ ${attacker.name} se met en position défensive !`));        
        default:          
          return message.reply(fonts.christus("❌ Commande invalide\n» a - Attaque\n» b - Technique\n» x - Ultime\n» c - Charger\n» d - Défense"));      }      
      ...
      if (state.turn === "p1") {        
        state.p1Chakra = Math.min(100, state.p1Chakra + state.chakraRegen);      } else {        
      ...
      async function sendBattleMessage() {        
        ...
        if (state.p1HP <= 0 || state.p2HP <= 0) {          
          ...
          delete gameState[threadID];        } else {          
          state.turn = state.turn === "p1" ? "p2" : "p1";          
