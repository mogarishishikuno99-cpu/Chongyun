...
module.exports = {
  ...
  langs: {
    en: {
      ...
      timeoutMessage: "⏰ 𝖳𝖾𝗆𝗉𝗌 é𝖼𝗈𝗎𝗅é ! 𝖡𝗈𝗇𝗇𝖾 𝗿é𝗽𝗼𝗻𝘀𝗲: {correctAnswer}",
  ...
  onStart: async function ({ message, event, args, commandName, getLang, api, usersData }) {
    try {
      ...
      return message.reply(f.christus("⚠️ Une erreur est survenue, réessayez."));
    }
  },
  ...
  async handleDefaultView(message, getLang) {
    try {
      ...
      return message.reply(f.christus(
        `🎯 𝗤𝘂𝗶𝘇\n━━━━━━━━\n\n` +
      ...
      return message.reply(f.christus("⚠️ Impossible de récupérer les catégories. Essayez 'quiz help' pour les commandes."));
    }
  },
  ...
  async handleRank(message, event, getLang, api, usersData) {
    try {
      ...
      if (!user || user.total === 0) {
        return message.reply(f.christus(`❌ Vous n'avez pas encore joué de quiz ! Utilisez 'quiz random' pour commencer.\n👤 Bienvenue, ${userName} !`));
      }
      ...
      return message.reply(f.christus("⚠️ Impossible de récupérer le classement. Réessayez plus tard."));
    }
  },
  ...
  async handleLeaderboard(message, getLang, args, api) {
    try {
      ...
      if (!rankings || rankings.length === 0) {
        return message.reply(f.christus("🏆 Aucun joueur dans le classement. Commencez à jouer pour être le premier !"));
      }
      ...
      return message.reply(f.christus("⚠️ Impossible de récupérer le classement. Le serveur est peut-être occupé, réessayez plus tard."));
    }
  },
  ...
  async handleCategories(message, getLang) {
    try {
      ...
      return message.reply(f.christus("⚠️ Impossible de récupérer les catégories."));
    }
  },
  ...
  async handleDailyChallenge(message, event, commandName, api) {
    try {
      ...
      const info = await message.reply(f.christus(
        `🌟 𝗗é𝗳𝗶 𝗾𝘂𝗼𝘁𝗶𝗱𝗶𝗲𝗻\n━━━━━━━━━\n\n` +
        `📅 ${challengeDate}\n` +
      ...
      setTimeout(() => {
        ...
        if (r) {
          message.reply(f.christus(`⏰ Temps écoulé ! La bonne réponse était: ${question.answer}`));
      ...
      return message.reply(f.christus("⚠️ Impossible de créer le défi quotidien."));
    }
  },
  ...
  async handleTrueOrFalse(message, event, commandName, api) {
    try {
      ...
      const info = await message.reply(f.christus(this.langs.en.torfReply.replace("{question}", question)));
      ...
      setTimeout(() => {
        ...
        if (reaction && !reaction.reacted) {
          ...
          message.reply(f.christus(this.langs.en.timeoutMessage.replace("{correctAnswer}", correctText)));
          message.unsend(info.messageID);
      ...
      return message.reply(f.christus("⚠️ Impossible de créer la question Vrai/Faux."));
    }
  },
  ...
  async handleFlagQuiz(message, event, commandName, api) {
    try {
      ...
      if (!Array.isArray(options) || !options.length) {
        return message.reply(f.christus("⚠️ Aucune question sur les drapeaux disponible pour le moment. Réessayez plus tard."));
      }
      ...
      setTimeout(() => {
        const r = global.GoatBot.onReply.get(info.messageID);
        if (r) {
          message.reply(f.christus(`⏰ Temps écoulé ! La bonne réponse était: ${answer}`));
          message.unsend(info.messageID);
          global.GoatBot.onReply.delete(info.messageID);
        }
      }, 30000);
      ...
      return message.reply(f.christus(`⚠️ Impossible de créer le quiz drapeaux.\n📄 Raison: ${detail}`));
    }
  },
  ...
  async handleAnimeQuiz(message, event, commandName, api) {
    try {
      ...
      if (!Array.isArray(options) || !options.length) {
        return message.reply(f.christus("⚠️ Aucune question anime disponible pour le moment. Réessayez plus tard."));
      }
      ...
      const detail = err?.response?.data?.error || err.message || "erreur inconnue";
      return message.reply(f.christus(`⚠️ Impossible de créer le quiz anime.\n📄 Raison: ${detail}`));
    }
  },
  ...
  async handleImageQuiz(message, event, commandName, category, title) {
    try {
      ...
      if (!Array.isArray(options) || !options.length) {
        return message.reply(f.christus(`⚠️ Aucune question « ${category} » disponible pour le moment. Réessayez plus tard.`));
      }

      const body = f.christus(
        `${title}\n━━━━━━━━\n\n❔ ${hint || question}\n\n` +
      ...
      const detail = err?.response?.data?.error || err.message || "erreur inconnue";
      return message.reply(f.christus(`⚠️ Impossible de créer le quiz ${category}.\n📄 Raison: ${detail}`));
    }
  },
  ...
  async handleQuiz(message, event, args, commandName, getLang, api, usersData, forcedDifficulty = null) {
    try {
      ...
      const body = f.christus(getLang("reply")
      ...
      setTimeout(() => {
        const r = global.GoatBot.onReply.get(info.messageID);
        if (r) {
          message.reply(f.christus(getLang("timeoutMessage").replace("{correctAnswer}", answer)));
          message.unsend(info.messageID);
          global.GoatBot.onReply.delete(info.messageID);
        }
      }, 30000);
      ...
      message.reply(f.christus("⚠️ Impossible de récupérer une question. Essayez 'quiz categories' pour voir les options disponibles."));
    }
  },
  ...
  async handleCategoryLeaderboard(message, getLang, args, api) {
    try {
      const category = args[0]?.toLowerCase();
      if (!category) {
        return message.reply(f.christus("📚 Veuillez spécifier une catégorie pour voir le classement."));
      }
      ...
      if (!users || users.length === 0) {
        return message.reply(f.christus(`🏆 Aucun joueur trouvé pour la catégorie: ${category}.`));
      }
      ...
      return message.reply(f.christus("⚠️ Impossible de récupérer le classement de la catégorie."));
    }
  },
  ...
  onReaction: async function ({ message, event, Reaction, api, usersData }) {
    try {
      ...
      if (timeSpent > 30) {
        return message.reply(f.christus("⏰ Temps écoulé !"));
      }
      ...
      try {
        ...
        if (isCorrect) {
          ...
          const torfWrongMessages = [
            ...
            "🔄 𝗣𝗮𝘀 𝘁𝗼𝘁𝗮𝗹𝗲𝗺𝗲𝗻𝘁 ! 𝗣𝗮𝗿𝗳𝗼𝗶𝘀 𝗰'𝗲𝘀𝘁 𝘂𝗻𝗲 𝗾𝘂𝗲𝗌𝗍𝗂𝗈𝗇 𝗱𝗲 𝗰𝗵𝗮𝗻𝗰𝗲 ! 🎲",
          ...
          message.reply(f.christus(
            `${randomTorfWrongMsg}\n` +
            `━━━━━━━━━\n\n` +
            `🎯 𝗕𝗼𝗻𝗻𝗲 𝗿é𝗽𝗼𝗻𝘀𝗲: ${correctText} ✅\n` +
            `👤 ${userName}\n` +
            `💔 𝗦é𝗿𝗶𝗲 𝗿é𝗶𝗻𝗶𝘁𝗶𝗮𝗹𝗶𝘀é𝗲\n\n` +
            `🔥 𝗣𝗿𝗼𝗰𝗵𝗮𝗶𝗻𝗲 𝗾𝘂𝗲𝗌𝗍𝗶𝗈𝗇 𝗯𝗶𝗲𝗻𝘁ô𝘁 ! 𝗔𝗹𝗹𝗲𝘇, 𝘃𝗼𝘂𝘀 𝗽𝗼𝘂𝘃𝗲𝘇 𝗹𝗲 𝗳𝗮𝗶𝗿𝗲 ! 🚀`
          ));
        }
      } catch (error) {
  ...
  onReply: async function ({ message, event, Reply, getLang, api, usersData }) {
    ...
    try {
      ...
      if (!["A", "B", "C", "D"].includes(ans)) {
        return message.reply(f.christus("❌ Veuillez répondre avec A, B, C ou D uniquement !"));
      }
      ...
      if (user.achievements && user.achievements.length > 0) {
        ...
        await message.reply(f.christus(
          `🏆 Succès débloqué !\n${achievementMsg}\n💰 +50,000 pièces bonus !\n✨ +100 XP bonus !`
        ));
        const userData = await usersData.get(event.senderID) || {};
      ...
      message.reply(f.christus(`⚠️ Erreur lors du traitement de votre réponse: ${errorMsg}`));
    }
  },
