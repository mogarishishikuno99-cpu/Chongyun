...
// ═══════════════════════════════════════════════════════════════════════════════
//  TOP SOVEREIGN v3.0 — Classement visuel ultime avec Pagination onReply intégrée
//  Auteur   : Christus x Shade
//  Canvas   : 1100 px wide × hauteur dynamique — pixel-perfect, rien ne déborde
//  Avatar   : Facebook Graph API via api.getUserInfo
  ...
  void_matrix: {
    ...
    bg(ctx,W,H){
      ...
      for(let x=0;x<W;x+=20){
        ...
        for(let j=0;j<len;j++){ctx.globalAlpha=(1-j/len)*.13;ctx.fillStyle="#00FF41";ctx.font="9px BF, monospace";ctx.fillText(chars[Math.floor(Math.random()*chars.length)],x,14+j*14);}
      }
      ctx.restore();
    },
...
const NAME_MAX  = 260;
const COL_TIER  = COL_NAME + NAME_MAX + 16;
const COL_BAR   = COL_TIER + 160;
const BAR_W     = 210;
...
module.exports = {
  ...
  onReply: async function ({ message, event, Reply, usersData, api }) {
    ...
    if (!pageUsers.length) return message.reply(f.christus("◆  Aucun utilisateur sur cette page."));
